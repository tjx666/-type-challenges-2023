...
type IsLowercase<S extends string> = Uppercase<S> extends S ? false : true;

type CapitalizeWords<
    S extends string,
    Previous extends string = 'A',
    Result extends string = '',
> = S extends `${infer First}${infer Rest}`
    ? IsLowercase<Previous> extends false
        ? IsLowercase<First> extends true
            ? CapitalizeWords<Rest, First, `${Result}${Uppercase<First>}`>
            : CapitalizeWords<Rest, First, `${Result}${First}`>
        : CapitalizeWords<Rest, First, `${Result}${First}`>
    : Result;
