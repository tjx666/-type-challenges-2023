...
type KeyOf<T, K = keyof T> = T extends readonly unknown[]
    ? K extends `${number}`
        ? K | `[${K}]`
        : never
    : K;

type ObjectKeyPaths<T extends object, K extends keyof T = KeyOf<T>> =
    | K
    | (K extends unknown
          ? T[K] extends object
              ? `${K & string}${T[K] extends readonly unknown[] ? '.' | '' : '.'}${ObjectKeyPaths<T[K]> & string}`
              : never
          : never);

type X = keyof (typeof ref)['person']['books'];
...
type Y = ObjectKeyPaths<typeof ref>;
//   ^?
